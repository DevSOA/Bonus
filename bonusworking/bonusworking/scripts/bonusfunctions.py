...
def create_pivot_data(Employee_dic,quarters,proration,taxes,cutoff,quarter_to_calculate,company_performances,Days_By_Quarter):
        ...
        with open(f"{data_path}\\calculation.txt","w") as f:
            for i in Employee_dic.keys():
                ...
                if  employee_current_id != first_id:
                    f.write("This must be the first time the calculation must be done....... This message is a HEADER\n")
                    data = first_data
                    First_calculation = 0
                    ...
                    f.write("This is not the first time... This message must be in the middle of the note\n")
                    phr=data[1]
                    previous_date = datetime.strptime(phr,"%Y-%m-%d-%H:%M") 
                    from_currency = first_data[4]
                    #to_currency = data[4]
                    if from_currency != to_currency:
                        print(f"Change for employee {employee_current_id}")
                    dpt = int(data[3])
                    Entity = int(data[2])
                    f.write(f"Previous Data:{data} ..... \nCurrent Employee: {first_data}... Previous and current row data Must be different but the same ID\n")
                ...
                convertion_rate = get_currency_exchange(from_currency,to_currency,end_date.strftime("%Y-%m-%d"))
                currency_dic[from_currency][to_currency] = convertion_rate
                print(currency_dic,"\n\n")
                f.write(f"Conversion rate {convertion_rate} ::: From currency {from_currency} to currency {to_currency}\n")
                ...
                bonus_amount = bonus_amount * convertion_rate
                total_amount = 0
                ...
                f.write(f"How much the employee {employee_current_id} must be paid for each day? --> {partial_calculation}\n")
                ...
                if type(previous_date)==int :
                    ...
                    if hr <= end_d:
                        ...
                        f.write(f"How many days have the employee {employee_current_id} worked for this period? --> {percen_quarter_worked}   ..... Must Be the FIRST CALCULATION\n")
                    ...
                    if hr  !=  previous_date:
                        if hr <=  quarters[quarter_to_calculate-1][1]:
                            ...
                            if First_calculation == 0:
                                First_calculation = 1
                                percen_quarter_worked += 1
                            f.write(f"How Many Days have Worked {employee_current_id} --> {percen_quarter_worked}\n")
                            #cal.write(f"Calculated: {already_calculated}, New Calculation:{new_calculation}, Real Calculation:{percen_quarter_worked}\n" )
                total_amount += partial_calculation * percen_quarter_worked 
                f.write(f"How much the employee {employee_current_id} must be paid for this calculation? --> {total_amount}\n")
                #cal.write(f"Subtotal: {total_amount} \t")
                total_amount = round(total_amount)
                f.write(f"Total amount the employee {employee_current_id} must be paid? --> {total_amount}\n\n\n")
                #cal.write(f"\nTotal:{total_amount}\n\n")
...
#printbonus_amount = bonus_amount * convertion_rate
    if data == first_data:
            ...
            phr=data[1]
            previous_date = datetime.strptime(phr,"%Y-%m-%d-%H:%M")
    convertion_rate = get_currency_exchange(from_currency,currency,end_date.strftime("%Y-%m-%d"))
'''
    ...
    try:
        ...
        for i in pivot.keys():
            ...
            for j in pivot[i].keys():
                ...
                #print(tax)
                convertion_rate = set_n_digits(  get_currency_exchange(currency,"USD",end_date)   )
                converted = set_n_digits(debit) * set_n_digits(convertion_rate)
                converted = set_n_digits(converted)
                tax_converted = set_n_digits(tax) * set_n_digits(convertion_rate)
                ...
                tax = round(tax,2)
                #print(tax)
                convertion_rate = round(  get_currency_exchange(currency,"USD",end_date) ,2  )
                converted = round(debit,2) * round(convertion_rate,2)
                converted = round(converted,2)
                tax_converted = round(tax,2) * round(convertion_rate,2)
            ...
            '''
            print(f"entity:{entity}, Dpt:{dpt}, Entity Total: {entity_total_s}, Total Taxes: { total_tax_s}, Total Converted : {converted_total_s}, Total Tax Converted: {tax_converted_total_s}, Conversion:{convertion_rate}")
        ...
        logger.info("Cant create journal")
