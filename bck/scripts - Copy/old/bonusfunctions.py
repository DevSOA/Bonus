...
def create_pivot_data(Employee_dic,quarters,proration,taxes,cutoff,quarter_to_calculate,company_performances,year):
    ...
    try:
            ...
            with open (f"{data_path}\\calculation.txt","a+") as cal:
                if int(bonus_amount) != 0:
                    for i in range(quarter_to_calculate-1,-1,-1):
                        ...
                        end_d = quarters[i][1]
                        cal.write(f"\n\n\n")
                        cal.write(f"Starting calculation for quarter {i+1} for id ---> {id}\n")
                        if type(previous_date)==int :
                            cal.write(f"First calculation for id ---> {id}\n")
                            cal.write(f"Hire: {hr} , Start: {start_d} ,  End: {end_d}\n")
                            percent_quarter_worked = calculate_quarter_proration(quarters[i][0],hr,quarters[i][1])
                            days_worked_in_period=calculate_days(quarters[i][0],hr,quarters[i][1])
                            cal.write(f"Days worked: {days_worked_in_period}\n")
                            cal.write(f"Percent worked for this quarter: {percent_quarter_worked*100}\n")
                            
                        else:
                            cal.write(f"Continuing the calculation for the id --> {id} \n")
                            cal.write(f"Hire: {hr} , Start: {start_d} , Previous:{previous_date}, End: {end_d}\n")
                            ...
                            if data[4] != first_data[4]:
                                ...
                                cal.write(f"exchange rate from {first_data[4]} to {data[4]} ; {conversion_rate}\n")
                                cal.write(f"exchange rate from {data[4]} to {first_data[4]} ; {conversion_rate2}\n")
                            if hr >= previous_date:
                                if previous_date <= quarters[i][0]:
                                    ...
                                    days_already_worked_in_period=calculate_days(start_d,previous_date,end_d)
                                    new_calculation_days = calculate_days(start_d,hr,end_d)
                                    real_worked_in_period = new_calculation_days - days_already_worked_in_period
                                    cal.write(f"Days worked: {real_worked_in_period}\n")
                                    ...
                                    cal.write(f"Calculated: {already_calculated}, New Calculation:{new_calculation}, Real Calculation:{percent_quarter_worked*100}\n" )
                            #else:
                            #    print("Already calculated")
                                    calculation_per_line= partial_calculation * percent_quarter_worked * float(proration[i])
                        cal.write(f"Partial Calculation (per line): {calculation_per_line}\n")
                        total_amount += partial_calculation * percent_quarter_worked * float(proration[i])
                        cal.write(f"Subtotal (Sum of partial Calculations) : {total_amount} \t\n")
                    total_amount = round(total_amount)
                    cal.write(f"\nTotal Bonus calculated: {total_amount} for employee id {id}\n\n")
        ...
        return "Error Cant create the pivot data due ---> {e} // "
    Save_Sheet(currency_dic,f"{ROOT_DIR}\\Data\\Bonus_Report_RaaS_{year}.xlsx",f"Currency Exchange", 0, 0)
...
def calculate_days(start_date,date_of_hire_rehire,end_date):
    if len(f"{start_date}") > 10 :
        start_date = f"{start_date}"[0:10]
    if len(f"{end_date}") > 10 :
        end_date = f"{end_date}"[0:10]
    ...
    try:
        if date_of_hire_rehire >= end_date:
            ...
            return (end_date - date_of_hire_rehire).days + 1
    except:
        return "Error cant calculate quarter proration // // "
