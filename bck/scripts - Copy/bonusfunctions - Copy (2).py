        ...
        with open(f"{data_path}\\calculation.txt","w") as f:
            for i in Employee_dic.keys():
                ...
                if  employee_current_id != first_id:
                    f.write(f"Starting calculations for the employee with id --> {employee_current_id}\n")
                    data = first_data
                    First_calculation = 0
                    ...
                    f.write(f"Continuing calculations for the employee with id --> {employee_current_id}\n")
                    phr=data[1]
                ...
                #f.write(f"Conversion rate {conversion_rate} ::: From currency {from_currency} to currency {to_currency}\n")
                if  "Fixed Percent" in first_data[5]:
                    f.write(f"Salary for employee {employee_current_id} : {first_data[6]}\n")
                    f.write(f"Percentage of Bonus for employee {employee_current_id} : {first_data[7]}\n")
                    bonus_amount= float(first_data[6]) * float(first_data[7])
                    f.write(f"Initial calculation: Bonus Amount * Percentage [{bonus_amount}]\n")
                    commission=0
                else:
                        commission = 1
                        bonus_amount = 0
                if from_currency != to_currency:
                    f.write(f"Conversion from {from_currency} to {to_currency} : {conversion_rate}\n")
                    bonus_amount = bonus_amount * conversion_rate
                    f.write(f"{from_currency} converted to {to_currency} --> {bonus_amount}\n")
                total_amount = 0
                #partial_calculation =  (float(bonus_amount) * company_performances) / 365
                f.write(f"Company Performance: {company_performances}\n")
                partial_calculation =  (float(bonus_amount) * (float(company_performances) / 100) ) / 365
                f.write(f"Partial Calculation = ( Initial Calculation [{bonus_amount}] *  ( Company performance [{company_performances}] / 100 ) ) / 365  ---> {partial_calculation}  :::: Salary per day \n") 
                ...
                if type(previous_date)==int :
                    ...
                    if hr <= end_d:
                        ...
                        f.write(f"Days Worked from {hr} to {quarters[quarter_to_calculate-1][1]} --> {percen_quarter_worked}\n")
                    ...
                    if hr  !=  previous_date:
                        if hr <=  quarters[quarter_to_calculate-1][1]:
                            ...
                            f.write(f"Days Worked from  {hr}  to {previous_date} --> {percen_quarter_worked}\n")
                ...
                f.write(f"Days [{percen_quarter_worked}]  * Daily Rate [{partial_calculation}]: {calculation_per_line}\n") 
                ...
                f.write(f"Rounded: {total_amount}\n\n\n")
